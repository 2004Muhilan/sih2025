/* Complete AES-128 Implementation */
#include <stdio.h>
#include <stdint.h>
#include <string.h>

/* AES S-box */
static const uint8_t sbox[256] = {
    
};

typedef struct {
    uint32_t round_keys[44];
} AES_CTX;

/* Galois field multiplication */
uint8_t gf_mul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        uint8_t hi_bit = a & 0x80;
        a <<= 1;
        if (hi_bit) a ^= 0x1b;
        b >>= 1;
    }
    return p;
}

/* AES SubBytes */
void aes_sub_bytes(uint8_t *state) {
    for (int i = 0; i < 16; i++) {
        state[i] = sbox[state[i]];
    }
}

/* AES ShiftRows */
void aes_shift_rows(uint8_t *state) {
    uint8_t temp[16];
    temp[0] = state[0]; temp[1] = state[5]; temp[2] = state[10]; temp[3] = state[15];
    temp[4] = state[4]; temp[5] = state[9]; temp[6] = state[14]; temp[7] = state[3];
    temp[8] = state[8]; temp[9] = state[13]; temp[10] = state[2]; temp[11] = state[7];
    temp[12] = state[12]; temp[13] = state[1]; temp[14] = state[6]; temp[15] = state[11];
    memcpy(state, temp, 16);
}

/* AES MixColumns */
void aes_mix_columns(uint8_t *state) {
    uint8_t temp[16];
    for (int i = 0; i < 4; i++) {
        uint8_t *col = state + 4*i;
        temp[4*i] = gf_mul(col[0], 2) ^ gf_mul(col[1], 3) ^ col[2] ^ col[3];
        temp[4*i+1] = col[0] ^ gf_mul(col[1], 2) ^ gf_mul(col[2], 3) ^ col[3];
        temp[4*i+2] = col[0] ^ col[1] ^ gf_mul(col[2], 2) ^ gf_mul(col[3], 3);
        temp[4*i+3] = gf_mul(col[0], 3) ^ col[1] ^ col[2] ^ gf_mul(col[3], 2);
    }
    memcpy(state, temp, 16);
}

/* AES AddRoundKey */
void aes_add_round_key(uint8_t *state, uint32_t *round_key) {
    for (int i = 0; i < 16; i++) {
        state[i] ^= (round_key[i/4] >> (24 - 8*(i%4))) & 0xff;
    }
}

/* AES Key Expansion */
void aes_key_expansion(AES_CTX *ctx, const uint8_t *key) {
    uint32_t rcon[10] = {0x01000000, 0x02000000, 0x04000000, 0x08000000, 0x10000000,
                         0x20000000, 0x40000000, 0x80000000, 0x1b000000, 0x36000000};
    
    for (int i = 0; i < 4; i++) {
        ctx->round_keys[i] = (key[4*i] << 24) | (key[4*i+1] << 16) | 
                             (key[4*i+2] << 8) | key[4*i+3];
    }
    
    for (int i = 4; i < 44; i++) {
        uint32_t temp = ctx->round_keys[i-1];
        if (i % 4 == 0) {
            temp = ((sbox[(temp >> 16) & 0xff] << 24) |
                    (sbox[(temp >> 8) & 0xff] << 16) |
                    (sbox[temp & 0xff] << 8) |
                    sbox[(temp >> 24) & 0xff]) ^ rcon[i/4 - 1];
        }
        ctx->round_keys[i] = ctx->round_keys[i-4] ^ temp;
    }
}

/* AES Encrypt */
void aes_encrypt(const uint8_t *plaintext, uint8_t *ciphertext, const uint8_t *key) {
    AES_CTX ctx;
    uint8_t state[16];
    
    aes_key_expansion(&ctx, key);
    memcpy(state, plaintext, 16);
    
    aes_add_round_key(state, ctx.round_keys);
    
    for (int round = 1; round < 10; round++) {
        aes_sub_bytes(state);
        aes_shift_rows(state);
        aes_mix_columns(state);
        aes_add_round_key(state, ctx.round_keys + 4*round);
    }
    
    aes_sub_bytes(state);
    aes_shift_rows(state);
    aes_add_round_key(state, ctx.round_keys + 40);
    
    memcpy(ciphertext, state, 16);
}

int main() {
    uint8_t key[16] = {0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6,
                       0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c};
    uint8_t plaintext[16] = "Hello AES-128!!!";
    uint8_t ciphertext[16];
    
    printf("=== AES-128 Encryption ===\n");
    aes_encrypt(plaintext, ciphertext, key);
    printf("Encrypted successfully\n");
    
    return 0;
}